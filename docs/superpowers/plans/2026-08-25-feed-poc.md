...
- **Порог отказа от подхода — 800 строк кода** без учёта тестов; комментарии и
  строки документации не считаются. Пересмотрен владельцем 2026-08-26 после
  задачи 7: прежние 600 ставились как признак «логика переросла клей, строим
  продукт вместо проверки», и этого не случилось — семь узких модулей, 133
  строки пояснений, ни своего фронтенда, ни своего API. Число оказалось плохим
  измерителем замысла. Перевалим за 800 — замысел пересматриваем, а не
  дописываем.
- **Дубль поста при обрыве ровно в момент ответа ленты принят как граница PoC.**
  У Mastodon API нет ключей идемпотентности; устранить это можно лишь сверкой
  ленты перед каждой публикацией, а она добавляет запрос на каждый обход и ещё
  код — против цели «минимум своего». Принято владельцем 2026-08-26. Важно, что
  **потерянная развилка при этом невозможна**: отметка ставится до публикации и
  снимается при отказе. Лишний пост виден глазом и удаляется вручную.
- **Тест, создающий `Mapping`, обязан его закрыть** (`m.close()` или фикстура):
  соединение SQLite иначе остаётся открытым, и прогон `pytest -W error`
  краснеет `ResourceWarning`. Прогон с предупреждениями-как-ошибками входит в
  приёмку каждой задачи, где есть тесты.
...
set -euo pipefail
CT=poh-feed
PASS="${FEED_ACCOUNT_PASSWORD:?задайте пароль учёток в окружении}"
...
for a in issue_agent openhands pr_agent howtodemo delivery harness; do
  TOK=$(curl -sS -X POST "$BASE/oauth/token" \
    -d grant_type=password -d username="$a@feed.local" -d password="$PASS" \
...
def test_reopening_a_closed_poll_is_refused(tmp_path: Path):
    """Закрытый опрос означает «решение уже отправлено». Воскресив его,
    следующий обход отправил бы то же действие второй раз."""
    m = Mapping(tmp_path / "m.db")
    m.remember_poll("P1", "S1", REPO, 149, CHOICES)
    m.close_poll("P1")
    m.remember_poll("P1", "S1", REPO, 149, CHOICES)
...
def test_reposting_does_not_erase_decision_time(tmp_path: Path):
    """Замер простоя — главная цифра PoC. Повторная публикация не должна
    ни сдвигать момент публикации, ни терять уже принятое решение."""
    m = Mapping(tmp_path / "m.db")
    m.mark_posted("P1", 1000.0)
    m.mark_decided("P1", 1360.0)
    m.mark_posted("P1", 9999.0)
...
class Mapping:
    ...
    def remember_poll(
        ...
        # Повторная запись не воскрешает закрытый опрос: закрытый означает
        # «решение по этой развилке уже отправлено», и его возвращение в
        # выборку привело бы к повторной отправке того же действия.
        self._db.execute(
    ...
    def mark_posted(self, poll_id: str, when: float) -> None:
        # Момент публикации развилки — ПЕРВАЯ публикация, а не последний
        # обход. Повтор не должен ни сдвигать его, ни стирать уже
        # зафиксированное решение: иначе замер простоя молча теряется.
        self._db.execute(
...
Ожидаемо: все зелёные — 13 в `test_render.py` и 7 в `test_mapping.py`.
...
**Обязательные свойства модуля** (закрепить тестами, а не только реализацией):
каждый вызов к ленте оборачивает отказ в `RuntimeError` с именем агента и
названием операции, **сохраняя исходную причину** через `raise ... from error`;
основной путь `post_media` проверяется на факт передачи аргументов, а не только
на ветку отказа; `in_reply_to` проверяется во всех методах, где он есть.
...
class Feed:
    ...
    def votes(self, poll_id: str) -> dict[str, int]:
        ...
        # видимость поста — `unlisted`: `private` был бы виден только
        # подписчикам, а подписка агента на агента уходит в ожидание
        # одобрения. Клиента для чтения выбираем детерминированно, по
        # отсортированному имени, а не через `next(iter(...))` — порядок
        # вставки в словарь это случайность реализации, а не контракт.
        reader = min(self._clients)
        try:
            data = self._clients[reader].poll(poll_id)
            return {o["title"]: int(o["votes_count"]) for o in data["options"]}
        except Exception as error:
            raise RuntimeError(
                f"лента: чтение голосов опроса {poll_id!r} клиентом "
                f"{reader!r} не удалось: {error}"
            ) from error
```
...
**Интерфейсы:**
- Отдаёт: `Parked(repo, issue, title, phase)`; чистые функции
  `phase_of(labels, issue=None)`, `is_parked`, `stale_decision_labels`,
  `parked_from(issues, problems=None)`; класс `GitHub(token, client=None)` с
  методами `parked(repos, problems=None)`, `recent_issues`, `comment`,
  `labels`, `add_label`, `remove_label`.

**Обязательные свойства модуля** (закрепить тестами):
`phase_of` при нескольких метках `phase:*` возбуждает `ValueError` с номером
задачи и списком меток — угадывать нельзя, иначе неверная фаза даст неверную
метку, которую система молча проигнорирует. `parked_from` ловит такой отказ
**поштучно**: сломанная задача пропускается с записью причины в `problems`, а
обход остальных продолжается (в системе, которую мы обслуживаем, один
недоступный репозиторий однажды уронил весь обход и дал 171 падение подряд).
Каждый метод `GitHub` оборачивает отказ в `RuntimeError` с операцией,
репозиторием и номером задачи, сохраняя причину через `raise ... from error`.
Клиент подставляется параметром — иначе склейка не тестируется без сети.
...
**Интерфейсы:**
- Потребляет: `Mapping.timings()` — пары «когда опубликована развилка, когда
  принято решение». Таблица `timing` и оба метода заведены задачей 3, вызовы
  расставлены задачей 7; здесь ничего к хранилищу не добавляется.
...
def test_report_says_nothing_when_no_data(tmp_path):
    assert "данных нет" in report(Mapping(tmp_path / "m.db"))
...
Запустить: `cd bridge && .venv/bin/pytest -v` и `cd bridge && .venv/bin/pytest -W error -q`
Ожидаемо: оба прогона зелёные, вывод чистый.

- [ ] **Шаг 5: Коммит**

```bash
git add bridge/src/bridge/metrics.py bridge/tests/test_metrics.py
git commit -m "feat: замер простоя контура"
```

- [ ] **Шаг 6: Запустить мост надолго и прожить неделю**
...
| Дефолт по таймауту не вредит? | сколько раз контур увёл задачу в эскалацию без ответа |

Помнить об искажении: **пока ноутбук спит, мост стоит**, и в эти часы метрика
простоя врёт. Считать только по активным окнам.

- [ ] **Шаг 7: Вынести вердикт PoC**

Дописать в `docs/poc-results.md` раздел «Решение»: продолжать с переносом на
стенд и заменой GoToSocial своей реализацией — либо закрыть, назвав, что
именно не сработало.
