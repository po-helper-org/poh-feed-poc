#!/usr/bin/env bash
# Видимость public, а не unlisted: unlisted голосуется, но не попадает ни в
# публичную ленту инстанса, ни в ленты по хэштегу — панель шорткатов на тегах
# оставалась бы пустой. То же значение стоит в bridge/src/bridge/feed.py,
# и расхождение между ними означало бы, что дымовая проверка проверяет не то,
# чем пользуются.
...
FRAG=$(curl -sS -X POST "$BASE/api/v1/statuses" -H "Authorization: Bearer $T" \
  ...
  -d 'content_type=text/markdown' -d 'visibility=public')
...
REP=$(curl -sS -X POST "$BASE/api/v1/statuses" -H "Authorization: Bearer $H" \
  ...
  -d 'visibility=public' -d "media_ids[]=$M1" -d "media_ids[]=$M2")
...
CODE=$(curl -s -o /tmp/both.json -w '%{http_code}' -X POST "$BASE/api/v1/statuses" \
  -H "Authorization: Bearer $H" -d 'status=и то и другое' -d 'visibility=public' \
